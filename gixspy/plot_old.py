...
import numpy as np
from loader import (extract_angle_from, __load_image, find_direct_beam_file_index)
...
def main(files: list[str] | tuple[str]) -> None:
    ...
    intensity_db = np.sum(__load_image(files[direct_beam_file_index]), axis=1)
    ...
    for ii, tif in enumerate(files):
        ...
        intensity = np.sum(__load_image(tif), axis=1) - intensity_db
    ...
    for ii, tif in enumerate(files):
        angle = str(extract_angle_from(tif))
        angles[ii] = float(angle)
        intensity = np.sum(__load_image(tif), axis=1)[start_ind:end_ind]
